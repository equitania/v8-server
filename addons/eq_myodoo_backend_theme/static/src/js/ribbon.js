...
openerp.eq_myodoo_backend_theme = function(instance) {
    ...
    var res = model.call('get_param', ['theme.ribbon.name']).then(
        function (name) {
            if (name && name != 'False') {
                ribbon.html(name);
                ribbon.show();
    ...
    var res = model.call('get_param', ['theme.ribbon.color']).then(
        function (strColor) {
    ...
    var res = model.call('get_param', ['theme.ribbon.background.color']).then(
